...
<html>
     ...
     <table class="mws-datatable-fn mws-table dataTable" id="DataTables_Table_1" aria-describedby="DataTables_Table_1_info"> 
      <thead> 
       <tr role="row">
        ...
        <th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1" colspan="1" aria-label="Browser: activate to sort column ascending">广告名</th>
        <th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1" colspan="1" aria-label="Engine version: activate to sort column ascending">广告图片</th>
        <th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1" colspan="1" aria-label="Engine version: activate to sort column ascending">广告详情</th>
      ...
      <tr class="odd"> 
        ...
        <td class="  sorting_1"><img src="{{$v->url}}" width="100"></td>
        ...
        <td class="  sorting_1">
          <center>
          <form action="/guanggao/{{$v->id}}" method="post">
            <button class="btn btn-success">删除</button>
            {{method_field("DELETE")}}
            {{csrf_field()}}
          </form>
          <a href="/guanggao/{{$v->id}}/edit" class="btn btn-info edit">修改</a>
          </center>
        </td>
       </tr>
       @endforeach
 ...
 <script type="text/javascript">
  $('.contrary').click(function(){
    ...
    // alert(status)
    $.get('/demo',{status:status,id:id},function(data){
        if(data){
        	// alert(data)
          if(status == 1){
            o.html('开启').attr('class','btn btn-info contrary');
          }else if(status == 0){
            o.html('禁用').attr('class','btn btn-danger contrary');
          }
        }
    });
  });
